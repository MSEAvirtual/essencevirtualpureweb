...
const SponorPopUp = ({ data, company, closeModal, setClose }) => {
  ...
  console.log("overher---", s);
  if (s?.type === "direct") {
    openBUrl();
    console.log("overherpopo---", s);
    // setClose(true);
    // closeModal();
    // useEffect(())
    return (
        ...
        <div className="ford-container">
          <div style={{ height: "100%", textAlign: "center", marginTop: "10%"}}>
            ...
            <div className="content-button">
              <Button className={`custom-btn curved mt-10 ${type === 2 ? "ford-color" : "ex-color"}`} onClick={closeModal}>Continue</Button>
              <span className={`close-btn ${type === 2 ? "ford-t-color" : "ex-t-color"}`} onClick={closeModal}>X</span>
            </div>
          </div>
        </div>
      </div>
